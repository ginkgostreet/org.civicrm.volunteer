...
$get_projects_query = db_query("SELECT DISTINCT project_id FROM leaderce_varl_civicrm.civicrm_volunteer_need WHERE is_active =1 AND project_id IS NOT NULL AND (DATE_ADD(start_time, INTERVAL duration MINUTE) > NOW() OR is_flexible =1 OR duration IS NULL)");
...
foreach($project_records as $project) {
    ...
    $project_detail_query = db_query("SELECT * FROM leaderce_varl_civicrm.civicrm_volunteer_project WHERE id = ".$project['project_id']);
...
foreach ($project_details_array as $key => $project) {
    ...
    $get_projects_needs_query = db_query("SELECT * FROM leaderce_varl_civicrm.civicrm_volunteer_need WHERE is_active =1 AND project_id = ".$project['id']." AND (DATE_ADD(start_time, INTERVAL duration MINUTE) > NOW() OR is_flexible =1 OR duration IS NULL)");
    ...
    if(isset($get_projects_needs) && !empty($get_projects_needs)) {
        foreach ($get_projects_needs as $need_key => $need) {
            ...
            $create_appeal = db_insert("leaderce_varl_civicrm.civicrm_volunteer_appeal")->fields($appeal_data)->execute();
